...
@implementation VerificationController
{
    ...
    VerifyCompletionHandler     _completionHandler;
    BOOL                        _isProcessing;
}
...
- (id)init
{
	...
	if (self != nil)
    {
        ...
        _completionHandler = ^(BOOL suc, SKPaymentTransaction* transaction){};
        _transaction = nil;
        _isProcessing = NO;
	}
	return self;
...
-(void) dealloc
{
    [transactionsReceiptStorageDictionary release];
...
{
    [self verifyPurchase:transaction serverURL:ITMS_PROD_VERIFY_RECEIPT_URL completionHandler:(VerifyCompletionHandler)completionHandler_];
}

- (void)verifyPurchase:(SKPaymentTransaction *)transaction serverURL:(NSString*)serverURL completionHandler:(VerifyCompletionHandler)completionHandler_
{
    if( _isProcessing == YES )
    {
//        NSAssert(_isProcessing==YES, @"PaymentTransactions(Receipt Verification) can not be processed simultaneously");
        return;
    }
    ...
    if (!isOk)
    {
        ...
        _completionHandler(FALSE, _transaction);
        return;
    }
    ...
    NSAssert(request!=nil, @"Invalid Request URL");
    ...
    _isProcessing = YES;
    _completionHandler = completionHandler_;
...
//    NSString *verifiedReceiptUniqueIdentifier       = [verifiedReceiptReceiptDictionary objectForKey:@"unique_identifier"];
    ...
    {
        NSAssert(false, @"check iOS version. Faile to Load 'identifierForVendor'");
    }
    #else
    {
        // Pre iOS 6 
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    _completionHandler(NO, _transaction);
    if( _transaction != nil )
    {
...
{
    NSError* error = nil;
    NSDictionary* resultDictionary = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves|NSJSONReadingMutableContainers|NSJSONReadingAllowFragments error:&error];
    int resultStatus = [[resultDictionary valueForKey:@"status"] integerValue];
    NSLog(@"%d", resultStatus);
    if( resultStatus == 21007 )
    {   // SandBoxMode, Send Again to Sandbox URL
        _isProcessing = NO;
        SKPaymentTransaction* oldTransaction = _transaction;
        [self verifyPurchase:_transaction serverURL:ITMS_SANDBOX_VERIFY_RECEIPT_URL completionHandler:_completionHandler];
    ...
    {   // Process Validation with ReceiptData
        ...
        if (isOk)
        {
            ...
            _completionHandler(YES, _transaction);
            NSLog(@"Validation successful");
            
            // Make a note of the fact that we've seen the transaction id already
            NSDictionary *receiptDict       = [self dictionaryFromPlistData:_transaction.transactionReceipt];
            NSString *transactionPurchaseInfo = [receiptDict objectForKey:@"purchase-info"];
            NSString *decodedPurchaseInfo   = [self decodeBase64:transactionPurchaseInfo length:nil];
            NSDictionary *purchaseInfoDict  = [self dictionaryFromPlistData:[decodedPurchaseInfo dataUsingEncoding:NSUTF8StringEncoding]];
            NSString *transactionId         = [purchaseInfoDict objectForKey:@"transaction-id"];
    ...
    {
        NSLog(@"Unknown Status, %@", resultDictionary.description);
//        NSAssert1(false, @"Unkown Status, %@", resultDictionary);
        _completionHandler(NO, _transaction);
        _isProcessing = NO;
        return;
    }
